<x-public-layout>
    ...
    @if($job->photos->where('status', 'processing')->count() > 0)
        <div class="mb-4 bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-200">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    @foreach($job->attachments as $attachment)
                        <a href="{{ URL::signedRoute('attachments.show', ['attachment' => $attachment->id, 'token' => $token]) }}" target="_blank" class="block">
                            @if(Str::startsWith($attachment->file_type, 'image/'))
                                <img src="{{ URL::signedRoute('attachments.show', ['attachment' => $attachment->id, 'token' => $token]) }}" alt="{{ $attachment->file_name }}" class="w-full h-24 object-cover rounded-lg">
                            @else
