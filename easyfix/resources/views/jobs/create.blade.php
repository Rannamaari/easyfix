<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2">
            <h2 class="font-semibold text-xl text-gray-900 leading-tight">Request a Service</h2>
            <p class="text-sm text-gray-500">Tell us what you need and we’ll connect you with the right provider.</p>
        </div>
    </x-slot>
                ...
                <form action="{{ route('jobs.store') }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-8">
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-xs text-gray-500">Tap an icon to select</span>
                        </div>
                        <div class="mt-3 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                            @foreach($categories as $category)
                                <button type="button"
                                    class="category-card border border-gray-200 rounded-xl p-4 text-left hover:border-blue-500 hover:bg-blue-50 transition dark:border-slate-700 dark:hover:border-blue-500 dark:hover:bg-slate-800"
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            <label class="block text-sm font-medium text-gray-700">Specific Service (optional)</label>
                            <span class="text-xs text-gray-500">Pick one or type your issue</span>
                        </div>
                        ...
                        @enderror
                        <div class="mt-3">
                            <label for="specific_issue" class="block text-sm font-medium text-gray-700">If not listed, describe your specific issue</label>
                    ...
                    <div class="space-y-4">
                        <div>
                            <p class="block text-sm font-medium text-gray-700">Service Address *</p>
                        ...
                        <div id="saved-address-fields" class="{{ $addressMode === 'saved' ? '' : 'hidden' }}">
                            <label for="address_id" class="block text-sm font-medium text-gray-700">Select Address</label>
                        ...
                        <div id="new-address-fields" class="{{ $addressMode === 'new' ? '' : 'hidden' }} space-y-4">
                            <div>
                                <label for="new_address_label" class="block text-sm font-medium text-gray-700">Address Type</label>
                            ...
                            <div>
                                <label for="new_address" class="block text-sm font-medium text-gray-700">Full Address</label>
                    ...
                    {{-- Attachments --}}
                    <div>
                        <label for="attachments" class="block text-sm font-medium text-gray-700">Photos (optional, max 5)</label>
                        <input type="file" name="attachments[]" id="attachments" multiple accept="image/*,.pdf"
                            class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                        <p class="mt-1 text-xs text-gray-500">Upload photos of the issue (JPG, PNG, PDF - max 5MB each)</p>
                        @error('attachments.*')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        ...
        function renderServices(services) {
            ...
            services.forEach(service => {
                ...
                btn.type = 'button';
                btn.className = 'service-chip px-3 py-1.5 rounded-full border border-gray-200 text-sm text-gray-700 hover:border-blue-500 hover:text-blue-700';
                btn.dataset.serviceId = service.id;
                btn.textContent = service.name;
                btn.addEventListener('click', () => {
                    serviceIdInput.value = service.id;
                    document.querySelectorAll('.service-chip').forEach(chip => chip.classList.remove('bg-blue-600', 'text-white', 'border-blue-600'));
        ...
        categoryCards.forEach(card => {
            card.addEventListener('click', () => {
                categoryCards.forEach(c => c.classList.remove('border-blue-600', 'ring-2', 'ring-blue-200'));
        ...
        if (categoryInput.value) {
            ...
            if (selectedCard) {
                selectedCard.classList.add('border-blue-600', 'ring-2', 'ring-blue-200');
