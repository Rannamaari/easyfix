@php
    $links = [
        ...
        ['label' => 'Services', 'href' => route('services.index'), 'active' => request()->routeIs('services.*'), 'icon' => 'squares-2x2'],
        ['label' => 'Projects', 'href' => route('subcontractor'), 'active' => request()->routeIs('subcontractor'), 'icon' => 'building-office-2'],
    ...
    $jobUpdateCount = auth()->check() && $user->isCustomer() ? $user->unreadJobUpdateCount() : 0;
@endphp
                        ...
                        <div id="user-menu"
                            ...
                            <div class="py-2">
                                ...
                                <a href="{{ route('jobs.index') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:text-slate-200 dark:hover:bg-slate-800/60">
                                    ...
                                    <span>My Jobs</span>
                                    @if($jobUpdateCount > 0)
                                        <span class="ml-auto inline-flex min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 py-0.5 text-[11px] font-semibold text-white">
                                            {{ $jobUpdateCount }}
                                        </span>
                                    @endif
                                </a>
            ...
            <div class="border-t border-gray-100 dark:border-slate-800 my-2 pt-2">
                @auth
                    ...
                    <a href="{{ route('jobs.index') }}" data-nav-link class="flex items-center gap-3 rounded-xl px-4 py-3 text-base font-medium text-gray-700 hover:bg-gray-50 dark:text-slate-300 dark:hover:bg-slate-800/50">
                        ...
                        <span>My Jobs</span>
                        @if($jobUpdateCount > 0)
                            <span class="ml-auto inline-flex min-w-6 items-center justify-center rounded-full bg-red-500 px-2 py-0.5 text-xs font-semibold text-white">
                                {{ $jobUpdateCount }}
                            </span>
                        @endif
                    </a>
