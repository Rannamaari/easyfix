...
class SyncMicronetJobStatusJob implements ShouldQueue
{
    ...
    public function handle(MicronetSyncService $syncService): void
    {
        ...
        try {
            $syncService->syncJobStatus($job);
        } catch (\Throwable $exception) {
            $job->forceFill(['micronet_sync_error' => $exception->getMessage()])->save();
            Log::warning('Micronet job status sync failed: ' . $exception->getMessage(), ['job_request_id' => $job->id]);
            throw $exception;
        }
    }
}
